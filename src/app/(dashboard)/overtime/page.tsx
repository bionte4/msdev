...
export default async function OvertimePage() {
  ...
  const [listResult, eligibilityResult] = await Promise.all([
    listOvertimeRequests(),
    getMyOvertimeEligibility(),
  ]);
  ...
  const eligible =
    eligibilityResult.success && eligibilityResult.data.eligible;
  const showForm =
    canSubmit && Boolean(session?.user.developerId) && eligible;

  return (
    <div className="page-stack">
      <PageHeader
        title="Overtime approvals"
        description="PENDING → APPROVED_CLIENT / REJECTED_CLIENT · blocked for lump-sum personnel"
      />

      {!session?.user.developerId && canSubmit && (
